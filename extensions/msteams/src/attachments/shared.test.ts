...
describe("msteams inline image limits", () => {
  ...
  it("emits an unavailable fact for inline data images above the per-image limit", () => {
    const attachments = [
      {
  ...
  it("emits an unavailable fact for inline data images with malformed base64 padding", () => {
    const attachments = [
      {
        contentType: "text/html",
  ...
  it("enforces cumulative inline size limit across attachments", () => {
    ...
    expect(out.map((candidate) => candidate.kind)).toEqual(["data", "unavailable"]);
  });
});
